...
import org.zaproxy.zap.extension.srm.SrmExtension;
...
public class InvalidCertificateDialogStrategy implements InvalidCertificateStrategy {
	...
	private final static String dialogTitle = Constant.messages.getString("srm.ssl.title");
	private final static String[] dialogButtons = { Constant.messages.getString("srm.ssl.reject"),
			Constant.messages.getString("srm.ssl.accepttemp"), Constant.messages.getString("srm.ssl.acceptperm") };
	...
	public CertificateAcceptance checkAcceptance(Certificate genericCert, CertificateException certError) {
		if (genericCert instanceof X509Certificate && defaultHostVerifier instanceof DefaultHostnameVerifier) {
			...
			message.add(new JLabel(Constant.messages.getString("srm.ssl.description")), gbc);
			...
			JLabel issuer = new JLabel(Constant.messages.getString("srm.ssl.issuer") + " ");
			...
			try {
				...
				JLabel mismatch = new JLabel(Constant.messages.getString("srm.ssl.mismatch") + " ");

...
import org.zaproxy.zap.extension.srm.ReportLastScan.ReportType;
...
public class UploadActionListener implements ActionListener{
	...
	public UploadActionListener(SrmExtension extension) {
	...
	public void generateAndUploadReport(){
		...
		try {
				...
				Thread uploadThread = new Thread(){
					...
					public void run(){
						...
						if(err != null)
							...
							View.getSingleton().showMessageDialog(Constant.messages.getString("srm.message.success"));
						reportFile.delete();
					}
				};
				uploadThread.start();
			...
			error = Constant.messages.getString("srm.error.failed");
	...
	public static String uploadFile(
		...
		if(responseCode == 400) {
			...
			err = Constant.messages.getString("srm.error.unsent") + " "
	...
	public static File generateReportFile(SrmExtension extension) throws Exception {
		File reportFile = File.createTempFile("srm-zap-report", ".xml");
